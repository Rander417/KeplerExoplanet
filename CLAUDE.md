...
* App code (`app/app.py`) reads only `app/data/*` and `app/model/*`; every number it shows must come from `kepler.app_bundle`. Label decisions go through `kepler.verdict`. Palette from `kepler.palette.THEMES` (light and dark sets, both validated; charts pick by `st.context.theme.type`, `st.plotly_chart(..., theme=None)`); plotly, not matplotlib; never `st.info` (it is blue) — use `st.container(border=True)`. Every widget and metric carries a plain-language `help=`; new jargon goes in the About-tab glossary.
