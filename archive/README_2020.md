# The 2020 README (preserved)

This is the original bootcamp-era write-up, moved here unchanged (apart from file paths) during the 2026 refresh. The current README is at the repository root; the refreshed results are in `notes/Research Log/`. Some statements below are known to be wrong and are corrected in the refresh — see `notes/Research Log/2026-08-24 Phase 2 review summary.md` — in particular the "f1" figures (they are accuracy / weighted f1; macro f1 is 0.77 / 0.87 / 0.87) and the habitable-zone method (period and stellar properties, never insolation).
...
- The DB & table definition SQL files are in `database/` (this folder) (moved in the 2026 refresh).
